...
class CashServiceIntegrationTest {
    ...
    void cashOperationDepositSuccess() {
        ...
        when(accountsClient.deposit(1L, "RUB", 1000.0)).thenReturn(5000.0);
        ...
        verify(accountsClient).deposit(1L, "RUB", 1000.0);
        verify(notificationsClient).sendNotification(eq(1L), eq("DEPOSIT"), anyString(), eq(1000.0), eq("RUB"));
    }

    @Test
    void cashOperationWithdrawSuccess() {
        CashOperationRequest request = new CashOperationRequest(1L, "WITHDRAW", "RUB", 500.0);

        when(blockerClient.checkOperation(any(CashOperationRequest.class))).thenReturn(false);
        when(accountsClient.withdraw(1L, "RUB", 500.0)).thenReturn(3500.0);
        ...
        verify(blockerClient).checkOperation(any(CashOperationRequest.class));
        verify(accountsClient).withdraw(1L, "RUB", 500.0);
        verify(notificationsClient).sendNotification(eq(1L), eq("WITHDRAW"), anyString(), eq(500.0), eq("RUB"));
    }

    @Test
    void cashOperationWhenBlocked() {
        CashOperationRequest request = new CashOperationRequest(1L, "WITHDRAW", "RUB", 150000.0);
        ...
        verify(blockerClient).checkOperation(any(CashOperationRequest.class));
        verify(accountsClient, never()).deposit(anyLong(), anyString(), anyDouble());
        verify(accountsClient, never()).withdraw(anyLong(), anyString(), anyDouble());
        verify(notificationsClient, never()).sendNotification(anyLong(), anyString(), anyString(), anyDouble(), anyString());
    }
    ...
    void cashOperationWhenAccountsClientFails() {
        ...
        when(blockerClient.checkOperation(any(CashOperationRequest.class))).thenReturn(false);
        when(accountsClient.deposit(1L, "RUB", 1000.0))
    ...
    void cashOperationWhenNotificationFails() {
        ...
        doThrow(new RuntimeException("Notification service unavailable"))
                .when(notificationsClient).sendNotification(anyLong(), anyString(), anyString(), anyDouble(), anyString());
    ...
    void cashOperationWithInvalidOperationType() {
        CashOperationRequest request = new CashOperationRequest(1L, "INVALID", "RUB", 1000.0);
