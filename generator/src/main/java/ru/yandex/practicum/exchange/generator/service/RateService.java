package ru.yandex.practicum.exchange.generator.service;

import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Service;

import java.math.BigDecimal;
import java.util.concurrent.ThreadLocalRandom;

@Service
@RequiredArgsConstructor
public class RateService {
    private final ExchangeProducer exchangeProducer;

    public void generateRates() {
        double rubToUsd = ThreadLocalRandom.current().nextDouble(60, 100);
        exchangeProducer.sendRate("RUB", "USD", BigDecimal.valueOf(rubToUsd));

        double usdToRub = rubToUsd - 5;
        exchangeProducer.sendRate("USD", "RUB", BigDecimal.valueOf(usdToRub));

        double rubToEur = ThreadLocalRandom.current().nextDouble(60, 100);
        exchangeProducer.sendRate("RUB", "EUR", BigDecimal.valueOf(rubToEur));

        double eurToRub = rubToEur - 5;
        exchangeProducer.sendRate("EUR", "RUB", BigDecimal.valueOf(eurToRub));
    }
}
